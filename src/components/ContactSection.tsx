    ...
    <section className="relative bg-transparent py-32 overflow-hidden">
      <div className="mx-auto max-w-7xl px-4 lg:px-6 relative z-10">
        ...
        <div className="max-w-4xl mb-20">
          <FadeUp delay={0.1}>
            <Heading level={2} className="text-[32px] lg:text-[52px] font-black tracking-tighter text-white mb-8">
              대한민국 물류 인프라의 기준, <br className="hidden sm:block" />
              라이드온과 함께 내일을 설계하십시오.
            </Heading>
          </FadeUp>
          <FadeUp delay={0.2}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24">
            ...
            <FadeUp delay={0.3}>
              <div className="p-8 rounded-[32px] bg-white/[0.03] border border-white/10 backdrop-blur-sm">
                ...
                <div className="space-y-4">
                  <a 
                    href="https://pf.kakao.com/_xxxx" // 실제 카카오톡 링크로 수정 필요
          ...
          {/* 우측: 파트너십 카테고리 (자랑하기 섹션) */}
          <div className="lg:col-span-7 grid grid-cols-1 gap-6">
            {partnerships.map((item, i) => (
              <FadeUp key={item.title} delay={0.4 + i * 0.1}>
                <div className="group flex items-start gap-6 p-8 rounded-3xl bg-white/[0.02] border border-white/5 transition-all hover:bg-white/[0.05] hover:border-brand-accent/20">
                  <div className="p-4 rounded-2xl bg-white/[0.05] group-hover:bg-brand-accent/10 transition-colors">
                    {item.icon}
                  </div>
                  <div>
        ...
        <FadeUp delay={0.8}>
          <div className="mt-20 pt-10 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-full border border-brand-accent/20 bg-brand-accent/5">
              <span className="text-[10px] font-bold" style={{ color: BRAND_COLOR }}>RIDE ON UNIVERSE</span>
            </div>
          </div>
        </FadeUp>
